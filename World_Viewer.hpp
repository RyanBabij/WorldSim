...
/* WorldSim: WorldViewer
	...
	Code for rendering the player's view of the world. It is able to render the global (world map) view, and additionally
	any local maps which have been loaded in.

	To do this it makes use of doubles. The main number tracks the global rendering coordinates, and the mantissa tracks
	the local tile rendering coordinates. There's probably a better way of doing this but it's the only way I know and it
	gets the job done.
	...
	It needs some cleanup and optimization. There are probably many things which could be done to improve performance,
	this thing can be slow on older computers when zoomed out too far. Also a lot of calculations should be moved out of
	the render function. It's worth noting that this is not a standard tile renderer. Most RPGs will render maybe a field
	of 20x20 tiles and therefore optimisation is not a concern. This one in some cases needs to render more tiles than 
	there are screen pixels.

	This class could technically be moved into Wildcat but it's not likely to be used for much else so it's not a big
	deal for now.
...
class WorldViewer: public DisplayInterface, public MouseInterface
{
	...
	*/
	...
	inline void setCenterTile (const double _centerTileX, const double _centerTileY,
	const int _subTileX=LOCAL_MAP_SIZE/2, const int _subTileY=LOCAL_MAP_SIZE/2)
	{
	...
	void renderWorldIcons()
	{
		...
		for (int i=0;i<world->vWorldObjectGlobal.size();++i)
		{
			...
			if ( tileSize > 4 && world->isSafe(tileX,tileY) )
			{
				...
				{
					if ( world->vWorldLocal(i2)->globalX == wog->worldX
						&& world->vWorldLocal(i2)->globalY == wog->worldY )
					{
						wog = 0;
						break;
					}
			...
			{
				Renderer::placeTexture4(wogPixelX,wogPixelY,wogPixelX+tileSize,wogPixelY+tileSize
				,wog->currentTexture(),false);
			}
		}
	...
	void render()
	{
		...
		if (lastMouseX >= mainViewX1 && lastMouseX <= mainViewX2
			&& lastMouseY >= mainViewY1 && lastMouseY <= mainViewY2 )
		{
	...
	// Render the local map for this world tile. This is most likely because
	// the player is active in this tile and looking at it.
				...
				for (int localXTile = 0; localXTile<LOCAL_MAP_SIZE;++localXTile)
				{
					if ( nextPixel>=mainViewX1 && currentPixel <= mainViewX2 && floor(currentPixel) != floor(nextPixel) )
					{
						...
						if ( subterraneanMode && localMap->dataSubterranean)
						{
							...
							// Unseen tiles
							if (FOG_OF_WAR && playerCharacter !=0 && activeMenu == MENU_ADVENTUREMODE
								&& playerCharacter->hasSeen(localMap, localXTile,localYTile,true) == 0 )
							{
							}
							//Previously seen tiles
							else if (FOG_OF_WAR && playerCharacter !=0 && activeMenu == MENU_ADVENTUREMODE
							&& playerCharacter->hasSeen(localMap, localXTile,localYTile,true) == 1 )
							{
								...
								if ( vText != 0)
								{
									...
									{
										Renderer::placeTexture4(currentPixel, currentSubY, ceil(nextPixel), ceil(nextSubY),
										(*vText)(i), false);
									}
								}
								delete vText;
							...
							else /* DRAW VISIBLE TILES */
							{
								...
								{
									Renderer::placeTexture4(currentPixel, currentSubY, ceil(nextPixel), ceil(nextSubY),
									localMap->dataSubterranean->aSubterranean
									(localXTile,localYTile).vObject(i)->currentTexture(), false);
								}
								delete vText;
							}
						}
						// Render non-subterranean mode
						else
						{
							//Very basic player line of sight check here (only if we're in Adventure mode)
							// Render tiles we have never seen before (basically leave them black)
							if (FOG_OF_WAR && playerCharacter !=0 && activeMenu == MENU_ADVENTUREMODE
								&& playerCharacter->hasSeen(localMap, localXTile,localYTile) == 0 )
							{
							}
							// Render tiles seen before but not currently seen.
							else if (FOG_OF_WAR && playerCharacter !=0 && activeMenu == MENU_ADVENTUREMODE
								&& playerCharacter->hasSeen(localMap, localXTile,localYTile) == 1 )
							{
								//Draw tile very dark to symbolise fog of war
							...
							// Render tiles we can currently see
							else
							{
								LocalTile* localTile = &localMap->data->aLocalTile(localXTile,localYTile);
								...
								/* This is an absolute shading system. It allows for 127 shades of height, and then wraps back
								down to the lowest. This seems to be a reasonably effective way of showing absolute heights
								which exceed 127, as the player can simply count out how many "rings" of wrapping there are to
								get a good idea of the elevation. */
								...
								else
								{
									...
									//if (currentSecond > 50 )
									//{
											//glColor3ub(80+(sunsetCounter*9)+(lightValue/2),80+(sunsetCounter*9)+(lightValue/2),
											//80+(sunsetCounter*10)+(lightValue/2));
									//}
								}
								...
								if (localTile->bWall != 0 )
								{
									if ( (localTile->bWall & 0b10001000) == 0b10001000) // NORTH
									{
										...
										(currentPixel, currentSubY, ceil(nextPixel), ceil(nextSubY),
										&TEX_WALL_GREYBRICK_SOUTH, 180);
									}
									if ( (localTile->bWall & 0b00010001) == 0b00010001) // WEST
									{
										Renderer::placeTexture4RotatedDegrees
										(currentPixel, currentSubY, ceil(nextPixel), ceil(nextSubY),
										&TEX_WALL_GREYBRICK_SOUTH, 90);
									}
									if ( (localTile->bWall & 0b01000100) == 0b01000100) // EAST
									{
										Renderer::placeTexture4RotatedDegrees
										(currentPixel, currentSubY, ceil(nextPixel), ceil(nextSubY),
										&TEX_WALL_GREYBRICK_SOUTH, 270);
									}
									if ( (localTile->bWall & 0b00100010) == 0b00100010) // SOUTH
									{
										Renderer::placeTexture4
										(currentPixel, currentSubY, ceil(nextPixel), ceil(nextSubY),
										&TEX_WALL_GREYBRICK_SOUTH, false);
									}
									// else if ( localTile->bWall == 0b01100110) // SE
									// {
									// Renderer::placeTexture4
									// (currentPixel, currentSubY, ceil(nextPixel), ceil(nextSubY),
									// &TEX_WALL_GREYBRICK_SE, false);
									// }
		...
		for (int currentY = pixelTileY; currentY<mainViewNY; currentY+=tileSize)
		{
			...
			for (int currentX = pixelTileX; currentX<mainViewNX+tileSize; currentX+=tileSize)
			{
				if (world->isSafe(tileX,tileY))
				{
					...
					// This tile needs to be rendered as a local map.
					...
					else if (landMode)
					{
						...
						{
							Renderer::placeTexture4(currentX, currentY, currentX+tileSize, currentY+tileSize,
							&TEX_WORLD_TERRAIN_OCEAN_00, false);
						}
						else if(world->isLand(tileX,tileY)==true)
						{
							Renderer::placeTexture4(currentX, currentY, currentX+tileSize, currentY+tileSize,
							&TEX_WORLD_TERRAIN_GRASS_00, false);
						}
					}
					// Render this tile as a world tile.
					else
					{
						/* Textures are chosen here rather than from tile objects because it is highly dependent on
						neighboring tiles. It would be possible to delegate texture handling to tile objects, but would take
						too much memory maintaining pointers to neighbours. In future I should probably write a class
						which returns joining textures based on neighbors. */

						// Todo:
						// tile should just return vector of textures tbh

						// UPDATE: Textures should be assigned to World_Local at generation/modification, to save cycles on
						// render time.
						World_Local * tile = &(world->aWorldTile(tileX,tileY));
						if (tile)
						{
							Renderer::placeTexture4(currentX, currentY, currentX+tileSize, currentY+tileSize,
							tile->currentTexture(), false);
						}

						// DRAW IMPROVEMENTS (FOREST, RIVER)
						// Improvements can layer over base terrain, and each other.
						// For example a tile may have a river and forest.
						// This should be handled by the tile
						if(world->isLand(tileX,tileY) && world->aWorldTile(tileX,tileY).baseBiome == FOREST)
						{
							Renderer::placeTexture4
							(currentX, currentY, currentX+tileSize, currentY+tileSize,
							&TEX_WORLD_TERRAIN_FOREST_TREES, false);
						}
						...
						world->aWorldTile(tileX,tileY).active==false)
						{
							Renderer::placeTexture4
							(currentX, currentY, currentX+tileSize, currentY+tileSize,
							&TEX_WORLD_TERRAIN_MOUNTAIN_00, false);
						}
